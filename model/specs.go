...
const (
	LainLabelPrefix = "cc.bdp.lain.deployd"
...
func (label ContainerLabel) NameAffinity() string {
	return fmt.Sprintf("affinity:%s.pg_name!=~%s", LainLabelPrefix, label.Name)
}

func (label ContainerLabel) Label2Maps() map[string]string {
	labelMaps := make(map[string]string)
	labelMaps[LainLabelPrefix+".pg_name"] = label.Name
	labelMaps[LainLabelPrefix+".pg_namespace"] = label.Namespace
	labelMaps[LainLabelPrefix+".instance_no"] = fmt.Sprintf("%d", label.InstanceNo)
	labelMaps[LainLabelPrefix+".version"] = fmt.Sprintf("%d", label.Version)
	labelMaps[LainLabelPrefix+".drift_count"] = fmt.Sprintf("%d", label.DriftCount)
	labelMaps[LainLabelPrefix+".container_index"] = fmt.Sprintf("%d", label.ContainerIndex)
	labelMaps[LainLabelPrefix+".annotation"] = label.Annotation
	return labelMaps
}
...
func (label *ContainerLabel) FromMaps(m map[string]string) bool {
	var err error
	hasError := false
	label.Name = m[LainLabelPrefix+".pg_name"]
	hasError = hasError || label.Name == ""
	label.Namespace = m[LainLabelPrefix+".pg_namespace"]
	label.InstanceNo, err = strconv.Atoi(m[LainLabelPrefix+".instance_no"])
	hasError = hasError || err != nil
	label.Version, err = strconv.Atoi(m[LainLabelPrefix+".version"])
	hasError = hasError || err != nil
	label.DriftCount, err = strconv.Atoi(m[LainLabelPrefix+".drift_count"])
	hasError = hasError || err != nil
	label.ContainerIndex, err = strconv.Atoi(m[LainLabelPrefix+".container_index"])
	hasError = hasError || err != nil
	label.Annotation = m[LainLabelPrefix+".annotation"]
	return !hasError
}
